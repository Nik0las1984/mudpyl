...
/*
  ...
  This file is a patched version of ordereddict 0.4.1; all code is by either
  the PSF or Anthon van der Neut.
*/
...
dump_ordereddict_head(register PyOrderedDictObject *mp)
{
	...
	if (PySortedDict_CheckExact(mp))
		...
		printf("ordereddict");
	printf(": fill %d, ", mp->od_fill);
	printf("used %d, ", mp->ma_used);
	printf("mask %d, ", mp->ma_mask);
	printf("mask %d, ", mp->ma_mask);
	printf("\nbits: ");
	if (KVIO(mp))
		printf("kvio ");
...
dictresize(PyOrderedDictObject *mp, Py_ssize_t minused)
{
	...
	for (epp = mp->od_otablep; j > 0; epp++, j--) {
